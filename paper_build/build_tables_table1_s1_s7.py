#!/usr/bin/env python3
"""
Build the canonical readable outputs for Table 1 and Tables S.1-S.7.

This script reads the canonical experiment workbooks under results/ and writes
publication-facing table files under paper_outputs/.
"""
...
from render_tables_readable import export_tables_from_frames
...
READABLE_DIR = TABLES_DIR / "readable"
...
def write_table_artifacts(
    ...
    stem = TABLE_FILE_STEMS[table_key]
    wide_flat = build_wide_flat(wide_df)

    (TABLES_DIR / f"{stem}_wide.csv").parent.mkdir(parents=True, exist_ok=True)
    wide_flat.to_csv(TABLES_DIR / f"{stem}_wide.csv", index=False)
    wide_flat.to_excel(TABLES_DIR / f"{stem}_wide.xlsx", index=False)
    long_df.to_csv(TABLES_DIR / f"{stem}_long.csv", index=False)
    long_df.to_excel(TABLES_DIR / f"{stem}_long.xlsx", index=False)
...
def main() -> None:
    global OUTPUT_DIR, TABLES_DIR, READABLE_DIR

    parser = argparse.ArgumentParser(description="Build paper tables.")
    parser.add_argument(
        "--results-root",
        type=Path,
        default=RESULTS_DIR,
        help="Directory containing current experiment outputs",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    parser.add_argument(
        "--export-tex",
        action="store_true",
        help="Also export a combined all_tables.tex file after CSV/XLSX outputs are built",
    )
    parser.add_argument(
        "--allow-missing",
        action="store_true",
    ...
    scad_current = load_current_scad_results(args.results_root)
    other_current = load_current_other_results(args.results_root)
    synthetic_current = load_current_synthetic_results(args.results_root)
    ...
    latex, wide_flat = write_table_artifacts("Table S.1", s1_wide, s1_df, export_tex=args.export_tex)
    readable_frames["table_s1"] = wide_flat
    if latex:
        latex_blocks.append(("Table S.1", latex))

    table1_wide, table1_long = build_city_metric_wide(
        scad_current, "Table 1", SCAD_METHOD_ORDER, LOW_RATIOS, allow_missing=args.allow_missing
    )
    latex, wide_flat = write_table_artifacts("Table 1", table1_wide, table1_long, export_tex=args.export_tex)
    readable_frames["table1"] = wide_flat
    if latex:
        latex_blocks.append(("Table 1", latex))

    s6_wide, s6_long = build_city_metric_wide(
        scad_current, "Table S.6", SCAD_METHOD_ORDER, HIGH_RATIOS, allow_missing=args.allow_missing
    )
    latex, wide_flat = write_table_artifacts("Table S.6", s6_wide, s6_long, export_tex=args.export_tex)
    ...
    for table_name, ratios in [
        ...
        wide, long_df = build_city_metric_wide(
            other_current, table_name, OTHER_METHOD_ORDER, ratios, allow_missing=args.allow_missing
        )
        latex, wide_flat = write_table_artifacts(table_name, wide, long_df, export_tex=args.export_tex)
        readable_frames[TABLE_FILE_STEMS[table_name]] = wide_flat
        if latex:
            latex_blocks.append((table_name, latex))

    s7_wide, s7_long = build_table_s7_wide(synthetic_current, allow_missing=args.allow_missing)
    latex, wide_flat = write_table_artifacts("Table S.7", s7_wide, s7_long, export_tex=args.export_tex)
    readable_frames["table_s7"] = wide_flat
    if latex:
        latex_blocks.append(("Table S.7", latex))

    export_tables_from_frames(readable_frames, READABLE_DIR)

    if args.export_tex:
        tex_lines = ["% Auto-generated by paper_build/build_tables_table1_s1_s7.py", ""]
    ...
    print(f"Readable tables written to: {READABLE_DIR}")
    print(f"Machine-readable tables written to: {TABLES_DIR}")
