...
Usage:
    python experiments/synthetic_experiment.py --zeta 0.01 --ratio 0.10 --experiments 10
"""
...
from scad_methods import (
    scad_method_step1_original,
    scad_method_step2_merged,
    scad_method_step4_iterative
)
...
# Configuration
# ═══════════════════════════════════════
DATA_DIR = Path(__file__).parent.parent / "data" / "synthetic_topologies"
...
def run_single_experiment(case: Dict, ratio: float, seed: int,
    ...
    # Step 1
    try:
        r1 = scad_method_step1_original(A_raw, b_raw)
        if 'x_hat' in r1:
    ...
    # Step 2
    try:
        r2 = scad_method_step2_merged(tree_merged, b_merged)
        if 'x_hat' in r2:
    ...
    # Step 4
    try:
        r4 = scad_method_step4_iterative(A_merged, b_merged, tree_merged, x_noisy_merged, add_high=True)
        if 'x_hat' in r4:
...
def main():
    ...
    if args.city is not None or args.zeta is not None:
        ...
        save_name = args.save or f"results/synthetic/synthetic_{case_slug(case)}_alpha{int(args.ratio * 100)}.xlsx"
    else:
        ...
        save_name = args.save or 'results/synthetic/table_s7_combined.xlsx'
    ...
    save_path = Path(__file__).parent.parent / save_name
