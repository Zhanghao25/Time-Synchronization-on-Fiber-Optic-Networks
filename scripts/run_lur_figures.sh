...
run_job() {
  ...
  conda run -n "${CONDA_ENV}" python -u experiments/compute_lur_metrics.py \
    --data "${dataset}" \
...
if [[ "${MODE}" == "full" && "${USE_FINAL_MANIFESTS}" == "1" ]]; then
  ...
  for c in $CITIES; do
    conda run -n "${CONDA_ENV}" python -u postprocess/merge_lur_results.py \
...
conda run -n "${CONDA_ENV}" python -u paper_build/build_figures_figure5_s5_s6.py \
