...
mkdir -p results/main_scad
mkdir -p results/other_methods
mkdir -p results/synthetic
mkdir -p paper_outputs/tables/readable
mkdir -p "$LOG_DIR"
...
refresh_tables() {
  ...
  mkdir -p /tmp
  flock -x /tmp/jasa_refresh_tables.lock -c "conda run -n '$CONDA_ENV' python -u paper_build/build_tables_table1_s1_s7.py${allow_arg}"
}
...
export_tex_if_requested() {
  ...
  conda run -n "$CONDA_ENV" python -u paper_build/build_tables_table1_s1_s7.py --export-tex
}
...
finish_stage() {
  ...
  if [[ -d "results/main_scad/shards/${PAPER_TAG}" ]]; then
    echo
    echo "Merging SCAD shard outputs for ${stage_name}..."
    conda run -n "$CONDA_ENV" python -u postprocess/merge_table_results.py --merge-shards scad \
      --shard-dir "results/main_scad/shards/${PAPER_TAG}" \
      --output-dir "results/main_scad"
  fi

  if [[ -d "results/other_methods/shards/${PAPER_TAG}" ]]; then
    echo
    echo "Merging other-method shard outputs for ${stage_name}..."
    conda run -n "$CONDA_ENV" python -u postprocess/merge_table_results.py --merge-shards other \
      --shard-dir "results/other_methods/shards/${PAPER_TAG}" \
      --output-dir "results/other_methods"
  fi

  if [[ -d "results/synthetic/shards/${PAPER_TAG}" ]]; then
    echo
    echo "Merging synthetic shard outputs for ${stage_name}..."
    conda run -n "$CONDA_ENV" python -u postprocess/merge_table_results.py --merge-shards synthetic \
      --shard-dir "results/synthetic/shards/${PAPER_TAG}" \
...
submit_scad_case() {
  ...
  local outfile="results/main_scad/scad_city${city}_ratio${ratio}.xlsx"
  local cmd="conda run -n ${CONDA_ENV} python -u experiments/scad_main_experiment.py \
...
submit_other_case() {
  ...
  local outfile="results/other_methods/othermethods_${matrix_type}_city${city}_ratio${ratio}.xlsx"
  local merge_flag=""
  ...
  local cmd="${env_prefix} conda run -n ${CONDA_ENV} python -u experiments/other_methods_experiment.py \
    --data data/city${city}.xlsx \
    --experiments ${n_exp} \
    --ratio ${ratio} \
    --base-seed 0 \
...
submit_synthetic_case() {
  ...
  local outfile="results/synthetic/synthetic_zeta${zeta_tag}_alpha${alpha_pct}.xlsx"
  ...
  local cmd="conda run -n ${CONDA_ENV} python -u experiments/synthetic_experiment.py \
    --zeta ${zeta} \
    --ratio ${ratio} \
    --experiments ${n_exp} \
    --base-seed 42 \
...
case "$TARGET" in
  ...
  *)
    ...
    echo "Usage: bash scripts/run_paper_tables.sh [smoke|full] [table1|table_s6|scad_tables|other_methods|supp_tables|synthetic|all]"
    exit 1
    ;;
esac
...
echo "  Tables: paper_outputs/tables"
echo "  Logs: $LOG_DIR"
